defmodule Voria2Web.ManageLive.Stations.Form do
    ...
    socket =
      case socket.assigns.live_action do
        :new ->
          ...
          case Voria2.Network.get_installation(
                 installation_id,
        ...
        :edit ->
          ...
          case Voria2.Network.get_station(id, actor: socket.assigns.current_user) do
            {:ok, station} ->
              ...
              installation =
                case station.installation_id do
                  nil ->
                    nil
              ...
              socket
              |> assign(:page_title, gettext("Edit %{name}", name: station.name))
              |> assign(:form, form)
              |> assign(:installation, installation)
      ...
      {:ok, station} ->
        message =
          if socket.assigns.live_action == :new,
            do: gettext("Station created."),
            else: gettext("Station updated.")

        {:noreply,
         socket
         |> put_flash(:info, message)
         |> push_navigate(to: ~p"/manage/stations/#{station.id}")}
  ...
  def render(assigns) do
    ...
    <div class="max-w-2xl">
      <.breadcrumb crumbs={[
        ...
        {if(@live_action == :new, do: gettext("New Station"), else: @station.name), nil}
      ]} />

      <.header>
        {@page_title}
        <:subtitle :if={@live_action == :new && @installation}>
          {gettext("Adding a station to %{name}.", name: @installation.name)}
        </:subtitle>
      </.header>
        ...
        <.form for={@form} phx-change="validate" phx-submit="save">
          <div class=" border border-base-300 bg-base-100 divide-y divide-base-300 overflow-hidden">
            <div class="px-6 py-4 bg-base-200/40">
              <h3 class="text-sm font-semibold">{gettext("Station Details")}</h3>
            </div>
            <div class="px-6 py-5 space-y-4">
              <.input
                ...
                type="text"
                label={gettext("Name")}
                placeholder={gettext("Main Station")}
              />
              <.input
                field={@form[:slug]}
                type="text"
